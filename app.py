"""
股票技術指標監控儀表板 v2.0
新增：名字登入 + Cookie 自動登入 + Supabase 自選股持久化
"""
...
# Cookie 管理器（在 set_page_config 之後立即初始化）
import extra_streamlit_components as stx
_cookie_mgr = stx.CookieManager(key="stock_app_cookies")
...
COOKIE_KEY        = "stock_app_user"
COOKIE_EXPIRES    = datetime(2035, 1, 1)
RSI_PERIOD        = 14
...
def _do_login(name: str) -> bool:
    """執行登入：寫入 session + cookie + 從 Supabase 載入自選股"""
    name = name.strip()
    if not name:
        return False
    ...
    _cookie_mgr.set(COOKIE_KEY, name, expires_at=COOKIE_EXPIRES)
    return True


def _do_logout():
    """執行登出：清除 cookie + session"""
    _cookie_mgr.delete(COOKIE_KEY)
    for k in ["username", "us_watchlist", "tw_watchlist", "last_refresh"]:
        st.session_state.pop(k, None)
    st.rerun()


def _check_cookie_login():
    """嘗試從 cookie 自動登入（同一瀏覽器免輸入名字）"""
    if "username" in st.session_state:
        return  # 本次 session 已登入，跳過
    stored = _cookie_mgr.get(COOKIE_KEY)
    if stored:
        us, tw = _load_watchlist(stored)
        st.session_state.username     = stored
        st.session_state.us_watchlist = us
        st.session_state.tw_watchlist = tw
        st.session_state.last_refresh = datetime.now()
...
def render_login_screen():
    ...
    with col:
        ...
        <div style='text-align:center; padding:3rem 0 1.5rem 0;'>
            ...
            <div style='font-size:1.5rem; font-weight:700; color:#1e2a3a; margin-top:0.5rem;'>
                股票技術指標儀表板
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_stock_table(watchlist: list):
    ...
    <div class="legend-row">
        <b>顏色圖例：</b>&ensp;
        <span style='background:#c6efce;color:#276221;padding:2px 8px;border-radius:4px;font-weight:700'>RSI &lt; 30</span> 超賣&ensp;
        <span style='background:#ffc7ce;color:#9c0006;padding:2px 8px;border-radius:4px;font-weight:700'>RSI &gt; 70</span> 超買&ensp;
        <span style='background:#e2efda;color:#375623;padding:2px 8px;border-radius:4px'>KD &lt; 20</span> 低檔&ensp;
        <span style='background:#fce4d6;color:#833c0b;padding:2px 8px;border-radius:4px'>KD &gt; 80</span> 高檔&ensp;
        🌟 KD 黃金交叉（兩線在 20 以下 K 上穿 D）
    </div>
    """, unsafe_allow_html=True)
...
def main():
    ...
    with hdr_l:
        st.markdown(
            '<div class="gradient-title">📈 股票技術指標儀表板</div>',
            unsafe_allow_html=True,
        )
        st.caption(
    ...
    # ── 美股 / 台股 分頁 ──────────────────────────
    tab_us, tab_tw = st.tabs(["🇺🇸  美股", "🇹🇼  台股"])

    with tab_us:
        st.subheader("自選股管理")
        render_watchlist_manager("us_watchlist", "輸入美股代號，例如：AAPL、MSFT")
        st.divider()
